...
q5_msa<- telemetry_new %>%
  ...
  rename(number_ptts = X5..What.has.been.the.average...platforms.or.PTTs.routinely.requiring.telemetry..past.3.years.) %>%
...
scale_fill_manual(values = c("#0D8F81", "#39AB7E", "#A9DC67", "#EDEF5D")) +
 theme_classic() +
  ...
  annotate("text", x = c(1,2,3,4), y = 700, label = c("","+2 outliers","+2 outliers",""), size = 4.7, fontface = 2) +
  annotate("text", x = c(1,2,3,4), y = 675, label = c("mean = 52.0","mean = 2478.3","mean = 1968.3","mean = 30.0"), size = 5, fontface = 2)
...
ggplot(q5_lo, aes(x=stringr::str_wrap(line_office,20), y=number_ptts, fill=line_office)) +
#  geom_violin(width = 1.2) + 
  geom_boxplot() +
  ylim(0,700) + 
scale_fill_manual(values = c("#0D8F81", "#39AB7E","#6EC574", "#A9DC67", "#EDEF5D")) +
 theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
  ...
  annotate("text", x = c(1,2,3,4,5), y = 700, label = c("","","","+1 outlier","+1 outlier"), size = 4.7, fontface = 2) +
  annotate("text", x = c(1,2,3,4,5), y = 675, label = c("mean = 27.0","mean = 353.0","mean = 214.8","mean = 475.0","mean = 8260.5"), size = 5, fontface = 2)
```

Question 6: Mission Area
```{r}
msa<- telemetry_new %>%
...
ggplot(msa, aes(x = stringr::str_wrap(mission_area,25), y = msa_count, fill = line_office)) +
  ...
  labs(x = "Mission Area", y = "Count", fill = "Line Office") + scale_fill_brewer(palette = "Set2")
```


Question 7: Environmental Variables
```{r}
variables<-telemetry_new %>%
  select(line_office, mission_area, X7..Environmental.variables.collected..pick.as.many.as.appropriate.) %>%
  rename(environmental_variables = X7..Environmental.variables.collected..pick.as.many.as.appropriate.) %>%
...
ggplot(variables_msa, aes(x = reorder(environmental_variables, -var_count), y = var_count, fill = stringr::str_wrap(mission_area,37))) +
   geom_bar(position = "stack", stat = "identity") +
   theme_classic() +
   theme(axis.title = element_text(face = "bold", size = rel(1.4)),
         axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
...
ggplot(variables_lo, aes(x = reorder(environmental_variables, -var_count), y = var_count, fill = line_office)) +
   geom_bar(position = "stack", stat = "identity") +
   theme_classic() +
   theme(axis.title = element_text(face = "bold", size = rel(1.4)),
         axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
...
Question 10: Platform Operating Environment
```{r}
pltfrm_env<- telemetry_new %>%
  select(mission_area, line_office, X10..Indicate.Platform.Operating.Environment..pick.one.row..air..ocean..land..cryosphere..and.pick.one.column..altitude.or.depth...Air., X10..Indicate.Platform.Operating.Environment..pick.one.row..air..ocean..land..cryosphere..and.pick.one.column..altitude.or.depth...Ocean., X10..Indicate.Platform.Operating.Environment..pick.one.row..air..ocean..land..cryosphere..and.pick.one.column..altitude.or.depth...Land., X10..Indicate.Platform.Operating.Environment..pick.one.row..air..ocean..land..cryosphere..and.pick.one.column..altitude.or.depth...Cryosphere.) %>%
...
ggplot(data = env_sum, aes(x = environment, y = value, fill = pcent_value)) +
  ...
  theme(axis.text = element_text(colour = "black", size = rel(1), face = "bold"),
        axis.title = element_text(colour = "black", size = rel(1.4), face = "bold"),
        legend.title = element_text(face = "bold", size = rel(1.4)),
...
attr<- telemetry_new %>%
  dplyr::select(line_office, mission_area, 40:52) %>%
  separate_rows(mission_area, sep = "; ") %>%
...
Q22-34: Heatmap by Mission Area
```{r}
...
attr_msa_heatmap <- ggplot(data = attr_MSA_long, aes(x = stringr::str_wrap(MSA,15), y = stringr::str_wrap(question, 35), fill = `% Importance`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "darkblue") +
  geom_tile(colour = "white", size = 0.25) +
...
attr_lo_heatmap <- ggplot(data = attr_LO_long, aes(x = stringr::str_wrap(LO, 6), y = stringr::str_wrap(question, 35), fill = `% Importance`)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "darkblue") +
  geom_tile(colour = "white", size = 0.25) +
...
Questions 30, 33, 34
```{r}
# Telemetry Attribute Importance Questions 36, 37, 38, 40
attr2<- telemetry_new %>%
  dplyr::select(line_office, mission_area, 48, 51, 52) %>%
  separate_rows(mission_area, sep = "; ") %>%
  mutate(across(everything(), ~ifelse(.=="","Not Applicable", as.character(.))))%>%
  pivot_longer(cols = starts_with("X"),
               names_to = "question",
...
post_2027<- telemetry_new %>%
  dplyr::select(line_office, mission_area, 54, 55, 56, 58) %>%
  separate_rows(mission_area, sep = "; ")
...
future_number_msa <- post_2027 %>%
  rename(number_future_platforms = X37..In.2027.and.beyond..my.program.will.likely.require..pick.response..number.of.telemetry.subscriptions...platforms) %>%
...
future_bandwidth_msa <- post_2027 %>%
  rename(future_bandwidth = X38..In.2027.and.beyond..my.program.s.telemetry.bandwidth.requirements.per.platform.will.be.....) %>%
...
ggplot(spatial_domain_msa, aes(x = reorder(geo_cov, geo_count), y = geo_count, fill = stringr::str_wrap(mission_area,50))) +
  ...
  labs(x = "Spatial Domain", y = "Count", fill = "Mission Area") + scale_fill_brewer(palette = "Set2")
...
ggplot(pltfrm_mobility_msa, aes(x = reorder(stringr::str_wrap(mobility,20), mobility_count), y = mobility_count, fill = stringr::str_wrap(mission_area,50))) +
   geom_bar(position = "stack", stat = "identity") +
   theme_classic() +
   theme(axis.title = element_text(face = "bold", size = rel(1.4)),
         axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
         ...
         legend.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
         legend.position = c(0.22, 0.9)) +
  labs(x = "Platform Mobility", y = "Count", fill = "Mission Area") + scale_fill_brewer(palette = "Set2")
...
ggplot(pltfrm_mobility_lo, aes(x = reorder(stringr::str_wrap(mobility, 20), mobility_count), y=mobility_count, fill=line_office)) +
   ...
   theme(axis.title = element_text(face = "bold", size = rel(1.4)),
         ...
         legend.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
         legend.position = c(0.16, 0.89)) +
  labs(x = "Platform Mobility", y = "Count", fill = "Line Office") + scale_fill_brewer(palette = "Set2")
```
...
utility<- telemetry_new %>%
  select(line_office, mission_area, 13, 14,15, 29, 30,31, 32,33, 34, 36, 37, 38,39) %>%
  mutate_all(na_if, "") %>%
...
position_hor_all<- utility %>%
  ...
  mutate(position_accuracy_horizontal = as.factor(position_accuracy_horizontal)) %>% na.omit() %>%
...
position_hor_LO %>%
  ...
  scale_color_manual(values = c("#245668","#0D8F81", "#39AB7E", "#A9DC67", "#EDEF5D"), labels = function(x) str_wrap(x, width = 35)) +
  theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
  ...
  scale_x_discrete(limits = rev)+ ylim(0,100) + guides(linetype = FALSE, color = guide_legend(title = "Line Office")) +
  labs(x = "Horizontal Position Accuracy")
...
position_vert_MSA %>%
  ...
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        ...
        legend.key.size = unit(1.2,'lines'),
        legend.position = c(0.24, 0.86)) +
  scale_x_discrete(limits = rev)+ ylim(0,100) + geom_hline(yintercept = 50, linetype = "dotted")+
  guides(linetype = FALSE, color = guide_legend(title = "Mission Area")) +
...
position_max_MSA %>%
  ...
  ylim(0,100) + geom_hline(yintercept = 50, linetype = "dotted")+scale_x_discrete(limits = rev(levels(position_max_MSA$position_max_time))) +
  guides(linetype = FALSE, color = guide_legend(title = "Mission Area")) +
...
position_max_lo %>%
  ...
  ylim(0,100) + geom_hline(yintercept = 50, linetype = "dotted")+scale_x_discrete(limits = rev(levels(position_max_lo$position_max_time))) +
...
ops_duration_LO %>%
  ...
  geom_line(size = 1.2)+
  scale_linetype_manual(values = c("dashed", "solid", "solid", "solid", "solid","solid"))+
  scale_color_manual(values = c("#245668", "#0D8F81", "#39AB7E", "#6EC574","#A9DC67"), labels = function(x) str_wrap(x, width = 40)) +
  theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
...
power_use_MSA %>%
  ...
  ylim(0,100) + geom_hline(yintercept = 50, linetype = "dotted") +
  guides(linetype = FALSE, color = guide_legend(title = "Mission Area")) +
...
power_use_LO %>%
  ...
  geom_line(size = 1.2)+
  scale_linetype_manual(values = c("dashed", "solid", "solid", "solid", "solid"))+
  scale_color_manual(values = c("#245668", "#0D8F81", "#39AB7E", "#A9DC67"), labels = function(x) str_wrap(x, width = 40)) +
  theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
        legend.title = element_text(face = "bold", size = rel(1.4)),
        legend.text = element_text(face = "bold", colour = "black", size = rel(1.1)),
        legend.key.size = unit(1.2,'lines'),
...
mass_MSA %>%
  ...
  geom_line(size = 1.2)+
  scale_linetype_manual(values = c("dashed", "solid", "solid", "solid", "solid"))+
  scale_color_manual(values = c("#245668", "#0D8F81","#A9DC67","#EDEF5D"), labels = function(x) str_wrap(x, width = 40)) +
  theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
...
mass_LO %>%
  ...
  geom_line(size = 1.2)+
  scale_linetype_manual(values = c("dashed", "solid", "solid", "solid", "solid", "solid"))+
  scale_color_manual(values = c("#245668", "#0D8F81","#A9DC67"), labels = function(x) str_wrap(x, width = 40)) +
  theme_classic() +
  theme(axis.title = element_text(face = "bold", size = rel(1.4)),
        axis.text = element_text(face = "bold", colour = "black", size = rel(1.2)),
        axis.line = element_line(size = 1),
...
bandwidth_all<- utility %>%
  ...
  group_by(bandwidth) %>%
  dplyr::summarize(survey_count = n()) %>% as.data.frame() %>%
  mutate(bandwidth = ordered(bandwidth, levels = c("480 bps","1200 bps","2400 bps","9.6 kps","128 kps","1200 kps","2400 kps","5000 kps",">5000 kps (including G3, G4, G5)"))) %>%
  arrange(bandwidth) %>%
  dplyr::summarize(mission_area = "All",
         line_office = "All",
         bandwidth = bandwidth,
...
bandwidth_MSA %>%
  ...
  scale_x_discrete(limits = rev, guide = guide_axis(n.dodge = 2)) + ylim(0,100) + 
  geom_hline(yintercept = 50, linetype = "dotted") +
  guides(linetype = FALSE, color = guide_legend(title = "Mission Area")) +
...
transmit_freq_MSA %>%
  ...
  scale_x_discrete(limits = rev) + geom_hline(yintercept = 50, linetype = "dotted") + ylim(0,100) +
  guides(linetype = FALSE, color = guide_legend(title = "Mission Area")) +
...
transmit_freq_LO %>%
  ...
  scale_x_discrete(limits = rev) + ylim(0,100) +
  geom_hline(yintercept = 50, linetype = "dotted") +
  guides(linetype = FALSE, color = guide_legend(title = "Line Office")) +
  labs(x = "Longest Communications Temporal Window to Transmit Data")
...
latency_LO %>%
  ...
  geom_hline(yintercept = 50, linetype = "dotted") + ylim(0,100)+
  guides(linetype = FALSE, color = guide_legend(title = "Line Office")) +
  labs(x = "Longest Temporal Latency")
```
...
tel_provider_current<- telemetry_new %>%
  select(line_office, mission_area, Adapted9) %>% 
  ...
  group_by(line_office) %>%
  summarize(length_lo = n(),
            provider = provider,
            provider_type = provider_type,
            total = total) %>%
  as.data.frame() %>%
  group_by(line_office, provider) %>%
  summarize(current_length_provider = n(),
...
tel_provider_future <-  telemetry_new %>%
  dplyr::select(line_office, mission_area, X36..What.statement.best.characterizes.your.program.s.future...post.2027..plans.for.telemetry.service.provider.s.) %>%
  ...
  as.data.frame() %>%
  group_by(line_office, provider) %>%
  summarize(future_length_provider = n(),
...
ggplot(data = tel_provider_comp_lo, aes(x = stringr::str_wrap(provider,15), y = stringr::str_wrap(line_office,10), fill = pcent_change)) +
  geom_tile() +
...
ggplot(data = tel_provider_comp_msa, aes(x = stringr::str_wrap(provider,15), y = stringr::str_wrap(mission_area,15), fill = pcent_change)) +
  geom_tile() +
  scale_fill_gradient2(midpoint = 0, low = "blue3", mid = "white", high = "red4") +
  geom_tile(colour = "white", size = 0.25) +
