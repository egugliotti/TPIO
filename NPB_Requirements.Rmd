...
#EOS Hierarchy
r<-GET(paste0(eores,"get_reports_eos_hierarchy",sep=""))
json_text<-content(r, as = "text")
systems_json<-fromJSON(json_text)
systems<-systems_json[['rpteosh']]
...
platforms$platform_acronym<- gsub(" - spacecraft","", platforms$platform_acronym)
platforms$platform_name<- gsub(" - Spacecraft","", platforms$platform_name)
platforms$platform_name<- gsub("-spacecraft","", platforms$platform_name)
...
#eor_ext_attributes
#We need the extended attributes so that we can filter the standard attributes and the requirements only for those that could be met from space (so none that have DAS or Flight Hours)
...
#eor_attribute
#This is where we get rid of some columns we don't need and filter the standard attributes to those that don't have DAS or Flight Hours
...
#Files for Lauraleen's Tableau Dashboard
##NPB Joins for Requirements Output (for Lauraleen)
This is the first of two files that Lauraleen needs to make for her Tableau dashboard. Annotations of what is being done are commented within the chunk below.
```{r}
#### System joins ####
#Here we are joining the NPB "system" all the way down to environmental parameters and selecting the unique NPB GCMD pathways
...
#  filter(system_acronym == "NPB") %>%
  ...
  mutate(gcmd_path = paste0(gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2)) %>% filter(gcmd_variable == "Imagery: Multispectral") %>% View()
...
#### Deal with reqs and gcmds to match up to systems ####
#Now we need to filter to have only ACTIVE requirements and filter rows in the requirements web service that have requirement ids but not requirement names
...
#Filter EOR gcmds so that only the ones present in the basic requirements are selected and select just the important columns in that file
...
#1. Full join the systems joins we did above to the EOR gcmds by the "gcmd_path" variable that we created in both files
#2. Join to the filtered ACTIVE requirements. I'm using full join a lot and filtering out where the system_name is NA because I want to be positive I'm getting everything and I feel better about that this way
#3. Join to the value tree information so that I can get the MSAs for each of the requirements
#4. Mutate the value tree information to have IWPI requirements say WRN_IWPI and WRN_RWX to say WRN_PWX
#5. Mutate the WMO, SPRWG, and USGEO SNWG requirements to say TBS in value tree information
#6. Create a variable with all the attribute information and if there is none, remove that requirement (this is particularly for objective levels)
#7. Rename columns to be what Lauraleen needs
#8. Write to a .csv file to send to Lauraleen
...
This is the second of two files that Lauraleen needs to make for her Tableau dashboard. Annotations of what is being done are commented within the chunk below.
```{r}
#1. Like before, we are going to join the NPB "system" to the environmental parameter but instead of joining it with requirements, we are more focused on getting all the Baseline Product Names and categories which we must "manually" put in using R because those are not in EORES
#2. We "manually" put in those product categories
#3. Separate the platform name so that we just have the Thematic Product Area without any of the other wording EORES has
#4. Separate the sensing element name (which we will rename Product Sub-Category) so that we don't have any of the extra wording EORES has
#5. Separate the environmental parameter name (which we will rename Baseline Product) so that we don't have any of the extra wording or the unique baseline product identifier EORES has
#6. Rename some of the columns to what Lauraleen needed them to be
#7. Write to a .csv file and send to Lauraleen 
...
#Tables for Lewis that are written to a word document table
##Summary by Baseline Product and Priority/Type
Tables with and without the total # of requirements per Baseline Product
```{r}
#Join the NPB "system" the the environmental parameter and get just the Baseline Product names without the unique IDs. Then you want to create the unique_path to get the distinct baseline product names with their gcmd pathways (including the sensing element names because there are a couple baseline products that have the same name but in different categories)
npb_system_joins2 <- systems %>%
  ...
  left_join(ep2se, by = "sensing_element_id") %>%
  left_join(ep, by = "environmental_parameter_id") %>%
  dplyr::select(system_name, sensing_element_name, environmental_parameter_name, gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2) %>%
  separate(environmental_parameter_name, c("Baseline Product", "extra"), extra = "drop", sep = " Baseline Product") %>%
  mutate(unique_path = paste0(sensing_element_name,`Baseline Product`,gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2)) %>%
  distinct(unique_path, .keep_all = TRUE) %>%
  dplyr::select(-unique_path, -extra, -system_name)


#Rename EOR GCMD column names so they match the EOS GCMD column names
npb_eor_gcmd2 <- eorgcmd %>%
  dplyr::select(requirement_id, primary_gcmd_topic, primary_gcmd_term, primary_gcmd_variable, primary_gcmd_variable_l2) %>%
...
#1.Join the NPB system information to requirements by the gcmd topic, term, variable, and level 2 variable
#2. Only select ACTIVE requirements that were in the filter EOR attribute information that we filtered in the first chunk of this markdown file
#3. Make the requirement_source say COURL if it didn't have WMO, SPRWG, or USGEO SNWG and have the requirement_source include the priority as well
#4. Write this to a .csv because Lewis asked for it, but I only did this for him once so I commented it out. You can delete the # to uncomment it and have it run.
reqs2npb <- npb_system_joins2 %>%
  ...
  dplyr::select(sensing_element_name, `Baseline Product`, requirement_id, requirement_name, requirement_source, requirement_priority, requirement_status, gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2) %>%
  mutate(requirement_source = replace(requirement_source, is.na(requirement_source), "COURL")) %>%
...
#Take data from above and create data formatted for the tables
#This table DOES include the number of overall requirements for that baseline product
#1. Group by the sub-category and baseline product so that I can get all the gcmd's associated with each product pasted in the same column. The collapse = "\n" makes it so with every new gcmd pathway, it goes to a new line
#2. You have to do as.data.frame() after each group_by so that it doesn't maintin that grouping moving forward
#3. Count the number of requirements by WMO, SNWG, SPRWG, Priority 1, Priority 2, and Priority 3 for each Baseline Product
#4. Mutate the requirement source and counts to be in the same column
summary_reqs2npb<- reqs2npb %>%
  ...
  summarize(`Relevant Environmental Variables` = paste("-", unique(gcmd_variable), collapse = "\n"),
            requirement_source = requirement_source) %>% as.data.frame() %>%
  ...
  mutate(`Requirement Source` = ifelse(is.na(requirement_source),"", `Requirement Source`)) %>%   as.data.frame() %>%
...
## Create Table % of Baseline Products with at least 1 matched EOR record that looks like this
...
npb_system_joins2 <- systems %>%
  ...
  left_join(ep2se, by = "sensing_element_id") %>%
  left_join(ep, by = "environmental_parameter_id") %>%
  dplyr::select(system_name, platform_name, sensing_element_name, environmental_parameter_name, gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2) %>%
  separate(environmental_parameter_name, c("Baseline Product", "extra"), extra = "drop", sep = " Baseline Product") %>%
...
#Join GCMD to requirements data and filter accordingly
...
#Joins NPB system info to EOR, do some data formatting
reqs2npb <- npb_system_joins2 %>%
  full_join(eorbasic_filter_separately, by = c("gcmd_topic","gcmd_term","gcmd_variable","gcmd_variable_l2")) %>%
  dplyr::select(platform_name, sensing_element_name, `Baseline Product`, requirement_id, requirement_name, requirement_source, requirement_priority, requirement_status, gcmd_topic, gcmd_term, gcmd_variable, gcmd_variable_l2) %>%
  mutate(requirement_source = replace(requirement_source, is.na(requirement_source), "COURL")) %>%
  mutate(requirement_source = ifelse(requirement_source == "COURL", paste0(requirement_source," ", requirement_priority), requirement_source)) %>% 
...
#Summarizing data to produce percentages
#1. Group by platform name (geophysical and foundational)
#2. Summarize data to get counts by each of the requirement sources
#3. Then summarize data to get percents
#4. Format the data to look like the example above
filter_npb_table <- reqs2npb %>%
  ...
  dplyr::select(product_category, requirement_source, pcent_source) %>%
...
##Output All Tables to Word Doc
```{r}
#The numbers inserted in the first four body_add_par commands are done by running several chunks below and just manually seeing how many rows there are and typing those in
read_docx() %>%
  body_add_par("Number of Baseline Products = 154 (27 Foundational & 127 Geophysical") %>%
  body_add_par("Number of Baseline Products with at least 1 requirement = 143") %>%
  body_add_par("Number of Baseline Products with no requirement match = 10") %>%
...
###Lewis Ask for:
####1. Total Unique Baseline Products in the Foundational and Geophysical Categories (should be 154: 27 Foundational, 127 Geophysical)
```{r}
####Unique Products by their attributes/unique ID (F = 43, G = 286)####
#I don't run this very often so it's commented out
#npb_system_joins2 <- systems %>%
#  filter(system_acronym == "NPB") %>%
#  left_join(platform2system, by = "system_id") %>%
#  left_join(platforms, by = "platform_id") %>%
...
####2. Number of Baseline Products with at least 1 EOR match based on GCMD variable = 143
```{r}
####data wrangling####
npb_system_joins2 <- systems %>%
  filter(system_acronym == "NPB") %>%
...
####Get table of baseline products with 1 eor match = 143####
...
####3. Number of Baseline Products with no matched EOR records = 10
```{r}
bp_no_eor <- systems %>%
  filter(system_acronym == "NPB") %>%
  left_join(platform2system, by = "system_id") %>%
